...
int main(int argc, char **argv) {
	...
	if (argc == 10) {
...
#else
		strInputFileName = "D:\\VisualStudio\\MOSSDET_c\\mattest.mat";
		detectorPath = "D:\\VisualStudio\\MOSSDET_c";
		m_strOutputDirectory = "D:\\VisualStudio\\MOSSDET_c";
		startTime = 0;
		...
		if (eoiName.compare("HFO+IES")== 0) {
			...
			std::vector<std::string> selectedUnipolarChanels, selectedBipolarChanels;
			SS_Detector ssDetector(strInputFileName, detectorPath, m_strOutputDirectory, 1, false, &selectedUnipolarChanels, &selectedBipolarChanels, eoiName, false, startTime, endTime, false, samplingRate, verbose, saveTextFile);
		...
		if (verbose) {
			std::cout << "The file MOSSDET_Detections.mat has a detected event on each column\n";
			std::cout << "First Row is detection class second row is start time and third row is end time\n\n";
			std::cout << "Detection Classes\n";
			std::cout << "Ripple = 1\n";
			std::cout << "FR = 2\n";
			std::cout << "IES = 3\n";
		}
