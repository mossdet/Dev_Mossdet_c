...
SS_Detector::SS_Detector(std::string strInputFileName, std::string detectorPath, std::string strDirName, int patIdx, int montage, std::vector<std::string> *selectedUnipolarChanels, std::vector<std::string> *selectedBipolarChanels,
...
long SS_Detector::readEEG() {
	...
	m_MASS_File = m_patientName.find("PSG") != std::string::npos;
	...
	IO_EDFPlus_SignalImporter_c eegReader_EDF(m_strFileName, m_selectedUnipolarChanels, m_selectedBipolarChanels);
	eegReader_EDF.readUnipolarLabels(m_unipolarLabels);
	eegReader_EDF.readMontageLabels(m_montageLabels);

	m_samplingRate = eegReader_EDF.getSamplingRate();
	if (m_MASS_File) {
	...
	m_fileAnalysisSamplesToRead = (m_analysisEndTime - m_analysisStartTime + 1) * m_samplingRate;// eegReader_EDF.getSampleCount();

	if (m_useMontage) {
		bool ok = eegReader_EDF.ReadReformattedSamples(m_fileAnalysisStartSample, m_fileAnalysisSamplesToRead, m_reformattedSignalSamples);
		while (!ok);
	}
	else {
		bool ok = eegReader_EDF.ReadSamples(m_fileAnalysisStartSample, m_fileAnalysisSamplesToRead, m_reformattedSignalSamples);
		while (!ok);
	}

	m_totalSampleCount = m_reformattedSignalSamples.size() > 0 ? m_reformattedSignalSamples.back().size() : 0;

	m_fileStartDateAndTime = eegReader_EDF.getDateAndTime();
...
bool SS_Detector::characterizeEEG() {
	...
	double normLength = 1 * 60 * 60.0;	//12 hours
